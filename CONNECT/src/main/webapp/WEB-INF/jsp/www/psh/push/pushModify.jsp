...
<section>
    ...
    <div class="mb-3">
        <button class="btn btn-primary" type="button" onclick="savePush()">저장</button>
		<c:if test="${not empty param.msgId}">
	        <button class="btn btn-outline-danger" type="button" onclick="deletePush()">삭제</button>
	    </c:if>
        <a class="btn btn-outline-secondary" href="/psh/push/pushList">목록</a>
    </div>
...
<script>
    ...
    function readPush(id) {
        ...
        $.ajax({
            ...
            success: function (map) {
                const result = map.result || map.push || map;
                if (!result) return;
    ...
    function savePush() {
        ...
        const formData = $("#pushForm").serializeObject();
        ...
        $.ajax({
            ...
            success: function () {
                location.href = "/psh/push/pushList";
            },
            error: function () {
                alert("저장 중 오류 발생");
            }
