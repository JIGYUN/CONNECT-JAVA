...
<section>
    ...
    <div class="mb-3">
        <a class="btn btn-outline-secondary" href="/pay/payment/paymentList">목록</a>
    </div>
...
<script>
    ...
    function selectPaymentList() {
        $.ajax({
            url: API_BASE + '/selectPaymentList',
            type: 'post',
            ...
            success: function (map) {
                ...
                if (!resultList.length) {
                    ...
                    for (let i = 0; i < resultList.length; i++) {
                        ...
                        html += "<tr onclick=\"goToPaymentModify('" + (r.paymentIdx) + "')\">";
                        html += "  <td class='text-right'>" + (r.paymentIdx ?? '') + "</td>";
                        ...
                        html += "            aria-label='항목 " + (r.paymentIdx ?? '') + " 삭제'";
                        html += "            onclick=\"event.stopPropagation(); deleteRow('" + (r.paymentIdx) + "')\">삭제</button>";
                        html += "  </td>";
                        html += "</tr>";
                    }
                }
    ...
    function goToPaymentModify(id) {
        let url = '/pay/payment/paymentModify';
        if (id) url += '?' + paymentId + '=' + encodeURIComponent(id);
        location.href = url;
    }
    ...
    function goToPaymentList() {
        location.href = '/pay/payment/paymentList';
    }
</script>
