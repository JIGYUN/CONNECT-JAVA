...
<script>
    // ▼ JavaGen 치환 포인트 유지
    const API_BASE = '/api/mmp/mapGrp';
    const mapGrpId = 'mapGrpIdx';
    ...
    function selectMapGrpList() {
        $.ajax({
            url: API_BASE + '/selectMapGrpList',
            type: 'post',
            ...
            success: function (map) {
                ...
                if (!resultList.length) {
                    ...
                    for (let i = 0; i < resultList.length; i++) {
                        ...
                        html += "<tr onclick=\"goToMapGrpModify('" + (r.mapGrpIdx) + "')\">";
                        html += "  <td class='text-right'>" + (r.mapGrpIdx ?? '') + "</td>";
                        ...
                        html += "            aria-label='항목 " + (r.mapGrpIdx ?? '') + " 삭제'";
                        html += "            onclick=\"event.stopPropagation(); deleteRow('" + (r.mapGrpIdx) + "')\">삭제</button>";
                        html += "  </td>";
                        html += "</tr>";
                    }
                }

                $('#mapGrpListBody').html(html);
            },
            error: function () {
                alert('목록 조회 중 오류 발생');
            }
    ...
    function deleteRow(id) {
        ...
        const sendData = {};
        sendData[mapGrpId] = id;

        $.ajax({
            url: API_BASE + '/deleteMapGrp',
            type: 'post',
            contentType: 'application/json',
            dataType: 'json',
    ...
    function goToMapGrpModify(id) {
        let url = '/mmp/mapGrp/mapGrpModify';
        if (id) url += '?' + mapGrpId + '=' + encodeURIComponent(id);
        location.href = url;
    }
