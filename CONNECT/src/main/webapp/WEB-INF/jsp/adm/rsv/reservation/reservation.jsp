...
<script>
    ...
    function selectReservationList() {
        $.ajax({
            url: API_BASE + '/selectReservationList',
            type: 'post',
            ...
            success: function (map) {
                ...
                if (!resultList.length) {
                    ...
                    for (let i = 0; i < resultList.length; i++) {
                        ...
                        html += "<tr onclick=\"goToReservationModify('" + (r.reservationId) + "')\">";
                        html += "  <td class='text-right'>" + (r.reservationId ?? '') + "</td>";
                        ...
                        html += "            aria-label='항목 " + (r.reservationId ?? '') + " 삭제'";
                        html += "            onclick=\"event.stopPropagation(); deleteRow('" + (r.reservationId) + "')\">삭제</button>";
                        html += "  </td>";
                        html += "</tr>";
                    }
                }

                $('#reservationListBody').html(html);
            },
            error: function () {
                alert('목록 조회 중 오류 발생');
            }
    ...
    function goToReservationModify(id) {
        let url = '/rsv/reservation/reservationModify';
        if (id) url += '?' + reservationId + '=' + encodeURIComponent(id);
        location.href = url;
    }
