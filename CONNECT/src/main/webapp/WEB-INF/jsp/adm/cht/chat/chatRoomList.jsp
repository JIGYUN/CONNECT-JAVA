...
<section>
    ...
    <div class="mb-3">
        <button class="btn btn-primary" type="button" onclick="goToChatRoomModify()">글쓰기</button>
        <button class="btn btn-outline-secondary" type="button" onclick="goToChatRoom()">통합</button>
    </div>
...
<script>
            ...
            success: function (map) {
                ...
                if (!resultList.length) {
                    ...
                    for (let i = 0; i < resultList.length; i++) {
                        ...
                        html += "<tr onclick=\"goToChatRoomModify('" + (r.chatRoomIdx) + "')\">";
                        html += "  <td class='text-right'>" + (r.chatRoomIdx) + "</td>";
                        html += "  <td>" + (r.title) + "</td>";
    ...
    function goToChatRoomModify(id) {
        let url = '/adm/cht/chatRoom/chatRoomModify';
        if (id) url += '?' + roomId + '=' + encodeURIComponent(id);
        location.href = url;
    }

    function goToChatRoom() {
        location.href = '/adm/cht/chatRoom/chatRoom';
    }
</script>
