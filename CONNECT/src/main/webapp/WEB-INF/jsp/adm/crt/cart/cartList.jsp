...
<section>
    <h2 class="mb-3">장바구니 목록</h2>

    <div class="mb-3">
        <button class="btn btn-primary" type="button" onclick="goToCartModify()">글쓰기</button>
        <button class="btn btn-outline-secondary" type="button" onclick="goToCart()">통합</button>
    </div>
...
<script>
    ...
    function goToCartModify(id) {
        let url = '/adm/crt/cart/cartModify';
        if (id) url += '?' + cartId + '=' + encodeURIComponent(id);
        location.href = url;
    }

    function goToCart() {
        location.href = '/adm/crt/cart/cart';
    }
</script>
