 ...
 */
@Controller
@RequestMapping("/adm/cht") // 예) /bbs
public class AdmChatMessageController {
    ...
    public String page(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        ...
        model.put("map", map);
        return "adm/cht/chat/chatMessage"; // 예) bbs/board/board.jsp
    }

    /** 목록 페이지 */
    @RequestMapping("/chatMessage/chatMessageList")
    public String list(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "adm/cht/chat/chatMessageList"; // 예) bbs/board/boardList.jsp
    }

    /** 등록/수정 페이지 */
    @RequestMapping("/chatMessage/chatMessageModify")
    public String modify(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "adm/cht/chat/chatMessageModify"; // 예) bbs/board/boardModify.jsp
    }
}
