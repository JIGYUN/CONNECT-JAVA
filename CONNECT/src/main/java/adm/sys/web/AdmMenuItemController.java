...
/**
 * @ClassName   : WebMenuItemController.java
...
public class AdmMenuItemController {

    /** 기본 페이지 */
    @RequestMapping("/menuItem/menuItem")
    public String page(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        ...
        model.put("map", map);
        return "adm/sys/menuItem/menuItem"; // 예) bbs/board/board.jsp
    }

    /** 목록 페이지 */
    @RequestMapping("/menuItem/menuItemList")
    public String list(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "adm/sys/menuItem/menuItemList"; // 예) bbs/board/boardList.jsp
    }

    /** 등록/수정 페이지 */
    @RequestMapping("/menuItem/menuItemModify")
    public String modify(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "adm/sys/menuItem/menuItemModify"; // 예) bbs/board/boardModify.jsp
    }
}
