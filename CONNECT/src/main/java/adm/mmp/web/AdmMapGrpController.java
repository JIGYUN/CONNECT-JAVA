...
/**
 * @ClassName   : WebMapGrpController.java
...
public class AdmMapGrpController {

    /** 기본 페이지 */
    @RequestMapping("/mapGrp/mapGrp")
    public String page(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        ...
        model.put("map", map);
        return "adm/mmp/mapGrp/mapGrp"; // 예) bbs/board/board.jsp
    }

    /** 목록 페이지 */
    @RequestMapping("/mapGrp/mapGrpList")
    public String list(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "adm/mmp/mapGrp/mapGrpList"; // 예) bbs/board/boardList.jsp
    }

    /** 등록/수정 페이지 */
    @RequestMapping("/mapGrp/mapGrpModify")
    public String modify(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "adm/mmp/mapGrp/mapGrpModify"; // 예) bbs/board/boardModify.jsp
    }
}
