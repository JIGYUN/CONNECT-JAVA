...
public class JavaReactGen7 {
        ...
        try {
            ...
            if (fn.fileExists(tplList)) {
                ...
                if (!JSP_OVERWRITE && fn.fileExists(out)) {
                    ...
                    String s = fx.apply(fn.readText(tplList));
                    s = applyJspListUiTokens(s);
                    fn.writeText(out, s);
            ...
            if (fn.fileExists(tplMod)) {
                ...
                if (!JSP_OVERWRITE && fn.fileExists(out)) {
                    ...
                    String s = fx.apply(fn.readText(tplMod));
                    s = applyJspModifyUiTokens(s);
                    fn.writeText(out, s);
                    System.out.println("생성: " + out);
                }
    ...
    private static UiColMeta parseUiCol(String colName) {
        ...
        // ✅ gen 태그가 없으면: 기본은 form만 노출, list/search는 숨김
        if (!"1".equals(m.props.get("_genDefined"))) {
            m.genForm = true;
            m.genList = false;
            m.genSearch = false;
        }
        ...
        // ✅ type 미지정이면, 컬럼명/타입 기반으로 checkbox 자동 판별
        String uiType = getProp(m, "type", "");
        if (isBlank(uiType)) {
            if (isYnLikeColumn(m.upperName) || isYnLikeDbType(m.type)) {
    ...
    private static boolean isYnLikeColumn(String upperName) {
        if (upperName == null) return false;
        return upperName.endsWith("_AT")
            || upperName.endsWith("_YN")
            || "USE_AT".equals(upperName)
            || "DEL_YN".equals(upperName)
            || "DELETE_YN".equals(upperName);
    }

    private static boolean isYnLikeDbType(String dbType) {
        if (dbType == null) return false;
        String t = dbType.toLowerCase();
        return t.contains("char(1)") || t.contains("varchar(1)") || t.contains("tinyint(1)");
    }
    ...
    private static void parseUiRemarkInto(UiColMeta m, String remark) {
        ...
        for (String raw : parts) {
            ...
            if (lower.startsWith("gen:") || lower.startsWith("gen=")) {
                ...
                m.props.put("_genDefined", "1"); // ✅ gen 태그 존재 표시
                continue;
            }
            ...
            // label
            if (!labelSet) {
                if (lower.startsWith("type=") || lower.startsWith("w=") || lower.startsWith("width=") || lower.startsWith("align=")) continue;
                m.label = seg;
                labelSet = true;
            }
        }
...
private static String buildJspFormFields(List<UiColMeta> cols, String indent) {
    ...
    for (UiColMeta c : cols) {
        ...
        // ✅ checkbox (Y/N)
        if (isCheckbox(c)) {
            String id = c.camelName + "_chk";
            sb.append(indent).append("<div class=\"form-group\" style=\"max-width: ")
                .append(maxWidth.isEmpty() ? "640" : maxWidth)
                .append("px;\">\n");
            sb.append(indent).append("    <label>").append(label).append("</label>\n");
            sb.append(indent).append("    <div class=\"custom-control custom-checkbox\">\n");
            sb.append(indent).append("        <input type=\"hidden\" name=\"").append(c.camelName).append("\" value=\"N\"/>\n");
            sb.append(indent).append("        <input type=\"checkbox\" class=\"custom-control-input\" id=\"").append(id)
                .append("\" name=\"").append(c.camelName).append("\" value=\"Y\"/>\n");
            sb.append(indent).append("        <label class=\"custom-control-label\" for=\"").append(id).append("\">Y</label>\n");
        ...
        sb.append(indent).append("    <input type=\"").append(inputType).append("\" class=\"form-control\" name=\"").append(c.camelName)
            .append("\" id=\"").append(c.camelName).append("\"/>\n");
        sb.append(indent).append("</div>\n\n");
    }
	...
	private static String buildJspReadBind(List<UiColMeta> cols, String indent) {
	    ...
	    for (UiColMeta c : cols) {
	        ...
	        if (isCheckbox(c)) {
	            String id = c.camelName + "_chk";
	            sb.append(indent)
	              .append("if ((result.").append(c.camelName).append(" || \"\").toString().toUpperCase() === \"Y\") {\n");
	            sb.append(indent).append("    $('#").append(id).append("').prop('checked', true);\n");
	            sb.append(indent).append("}\n");
	            continue;
	        }
	...
	private static String buildJspListTh(List<UiColMeta> cols, String indent) {
	    ...
	    for (UiColMeta c : cols) {
	        ...
	        if (!isBlank(w)) {
	            ...
	            if (c.colName != null && c.colName.equalsIgnoreCase(db_key)) st.append("width: 90px; ");
	            else if (isDateLike(c.upperName)) st.append("width: 220px; ");
	            else if (c.upperName.contains("USER") || c.upperName.contains("WRITER")) st.append("width: 160px; ");
	        }
	        ...
	        sb.append(indent).append("<th class=\"text-nowrap\"").append(styleAttr).append(">").append(label).append("</th>\n");
	    }
	    if (sb.length() > 0) sb.setLength(sb.length() - 1);
	    return sb.toString();
	}
    ...
    private static HashMap<String, String> getTableMap() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String sTypeStr;
                if ("CLOB".equalsIgnoreCase(typeName) || "BLOB".equalsIgnoreCase(typeName)) {
                    sTypeStr = "clob";
                } else if (typeName != null && typeName.toUpperCase().matches(".*(INT|NUMBER|LONG).*")) {
                    sTypeStr = "int";
                } else {
                    sTypeStr = "string";
                }
