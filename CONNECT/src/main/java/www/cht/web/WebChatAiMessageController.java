...
/**
 * @ClassName   : WebChatMessageController.java
...
public class WebChatAiMessageController {

    /** 기본 페이지 */
    @RequestMapping("/chatMessage/chatMessage2")
    public String page(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        ...
        model.put("map", map);
        return "cht/chat/chatMessage"; // 예) bbs/board/board.jsp
    }

    /** 목록 페이지 */
    @RequestMapping("/chatMessage/chatAiMessageList")
    public String list(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "cht/chat/chatAiMessageList"; // 예) bbs/board/boardList.jsp
    }

    /** 등록/수정 페이지 */
    @RequestMapping("/chatMessage/chatMessageModify2")
    public String modify(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "cht/chat/chatMessageModify"; // 예) bbs/board/boardModify.jsp
    }
}
