...
public class WebChatAiRoomController {

    /** 기본 페이지 */
    @RequestMapping("/chatRoom/chatRoom2")
    ...
    /** 목록 페이지 */
    @RequestMapping("/chatRoom/chatAiRoomList")
    public String list(ModelMap model, @RequestParam HashMap<String,Object> map) throws Exception {
        if (UserSessionManager.isUserLogined()) {
            model.put("userVO", UserSessionManager.getLoginUserVO());
        }
        model.put("map", map);
        return "cht/chat/chatAiRoomList"; // 예) bbs/board/boardList.jsp
    }

    /** 등록/수정 페이지 */
    @RequestMapping("/chatRoom/chatRoomModify2")
