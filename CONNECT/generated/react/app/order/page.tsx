// filepath: src/app/order/page.tsx
'use client';

import { useEffect, useMemo, useRef, useState, type ComponentType } from 'react';
import { useSearchParams } from 'next/navigation';
import { useOrderByDate, useUpsertOrder } from '@/shared/order';
import type { OrderEntry, OrderUpsertInput } from '@/shared/order';
...
function pickHtmlSafe(v: OrderEntry | null | undefined): string {
    if (!v) return '';
...
export default function OrderPage() {
    ...
    const q: UseQueryResult<OrderEntry | null, Error> = useOrderByDate({
        diaryDt: selected,
        grpCd,
        ownerId,
    });

    const upsert: UseMutationResult<void, Error, OrderUpsertInput, unknown> = useUpsertOrder({ grpCd, ownerId });
    ...
    return (
        <div className="order-root" style={{ maxWidth: PAGE_MAX_WIDTH, margin: '0 auto', padding: 16 }}>
            {/* 헤더 */}
            ...
            <div className="calendar-wrap" style={{ border: '1px solid #e9edf3', borderRadius: 12, overflow: 'hidden', marginBottom: 12 }}>
                <table className="order-calendar">
            ...
            <style jsx>{`
                ...
                .order-calendar { width: 100%; table-layout: fixed; border-collapse: collapse; }
                .order-calendar thead tr { background: #f8fafc; color: #0f172a; }
                .order-calendar th { padding: 10px 0; font-weight: 700; }
                .order-calendar td {
