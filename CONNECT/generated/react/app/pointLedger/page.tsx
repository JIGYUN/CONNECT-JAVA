// filepath: src/app/pointLedger/page.tsx
'use client';

import { useEffect, useMemo, useRef, useState, type ComponentType } from 'react';
import { useSearchParams } from 'next/navigation';
import { usePointLedgerByDate, useUpsertPointLedger } from '@/shared/pointLedger';
import type { PointLedgerEntry, PointLedgerUpsertInput } from '@/shared/pointLedger';
...
function pickHtmlSafe(v: PointLedgerEntry | null | undefined): string {
    if (!v) return '';
...
export default function PointLedgerPage() {
    ...
    const q: UseQueryResult<PointLedgerEntry | null, Error> = usePointLedgerByDate({
        diaryDt: selected,
        grpCd,
        ownerId,
    });

    const upsert: UseMutationResult<void, Error, PointLedgerUpsertInput, unknown> = useUpsertPointLedger({ grpCd, ownerId });
    ...
    const onSave = async () => {
        const body: PointLedgerUpsertInput = {
            diaryDt: selected,
            content: content || '',
            grpCd,
            ownerId,
    ...
    return (
        <div className="pointLedger-root" style={{ maxWidth: PAGE_MAX_WIDTH, margin: '0 auto', padding: 16 }}>
            {/* 헤더 */}
            ...
            <div className="calendar-wrap" style={{ border: '1px solid #e9edf3', borderRadius: 12, overflow: 'hidden', marginBottom: 12 }}>
                <table className="pointLedger-calendar">
            ...
            <style jsx>{`
                ...
                .pointLedger-calendar { width: 100%; table-layout: fixed; border-collapse: collapse; }
                .pointLedger-calendar thead tr { background: #f8fafc; color: #0f172a; }
                .pointLedger-calendar th { padding: 10px 0; font-weight: 700; }
                .pointLedger-calendar td {
