...
import { useChatMessageByDate, useUpsertChatMessage } from '@/shared/chatMessage';
import type { ChatMessageEntry, ChatMessageUpsertInput } from '@/shared/chatMessage';
...
function pickHtmlSafe(v: ChatMessageEntry | null | undefined): string {
    if (!v) return '';
...
export default function ChatMessagePage() {
    ...
    const q: UseQueryResult<ChatMessageEntry | null, Error> = useChatMessageByDate({
        diaryDt: selected,
        grpCd,
        ownerId,
    });

    const upsert: UseMutationResult<void, Error, ChatMessageUpsertInput, unknown> = useUpsertChatMessage({ grpCd, ownerId });
    ...
    const onSave = async () => {
        const body: ChatMessageUpsertInput = {
            diaryDt: selected,
            content: content || '',
            grpCd,
            ownerId,
    ...
    return (
        <div className="chatMessage-root" style={{ maxWidth: PAGE_MAX_WIDTH, margin: '0 auto', padding: 16 }}>
            {/* 헤더 */}
            ...
            <div className="calendar-wrap" style={{ border: '1px solid #e9edf3', borderRadius: 12, overflow: 'hidden', marginBottom: 12 }}>
                <table className="chatMessage-calendar">
            ...
            <style jsx>{`
                ...
                .chatMessage-calendar { width: 100%; table-layout: fixed; border-collapse: collapse; }
                .chatMessage-calendar thead tr { background: #f8fafc; color: #0f172a; }
                .chatMessage-calendar th { padding: 10px 0; font-weight: 700; }
                .chatMessage-calendar td {
