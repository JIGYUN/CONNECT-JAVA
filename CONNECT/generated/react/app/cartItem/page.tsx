// filepath: src/app/cartItem/page.tsx
'use client';

import { useEffect, useMemo, useRef, useState, type ComponentType } from 'react';
import { useSearchParams } from 'next/navigation';
import { useCartItemByDate, useUpsertCartItem } from '@/shared/cartItem';
import type { CartItemEntry, CartItemUpsertInput } from '@/shared/cartItem';
...
function pickHtmlSafe(v: CartItemEntry | null | undefined): string {
    if (!v) return '';
...
export default function CartItemPage() {
    ...
    const q: UseQueryResult<CartItemEntry | null, Error> = useCartItemByDate({
        diaryDt: selected,
        grpCd,
        ownerId,
    });

    const upsert: UseMutationResult<void, Error, CartItemUpsertInput, unknown> = useUpsertCartItem({ grpCd, ownerId });
    ...
    const onSave = async () => {
        const body: CartItemUpsertInput = {
            diaryDt: selected,
            content: content || '',
            grpCd,
            ownerId,
    ...
    return (
        <div className="cartItem-root" style={{ maxWidth: PAGE_MAX_WIDTH, margin: '0 auto', padding: 16 }}>
            {/* 헤더 */}
            ...
            <div className="calendar-wrap" style={{ border: '1px solid #e9edf3', borderRadius: 12, overflow: 'hidden', marginBottom: 12 }}>
                <table className="cartItem-calendar">
            ...
            <style jsx>{`
                ...
                .cartItem-calendar { width: 100%; table-layout: fixed; border-collapse: collapse; }
                .cartItem-calendar thead tr { background: #f8fafc; color: #0f172a; }
                .cartItem-calendar th { padding: 10px 0; font-weight: 700; }
                .cartItem-calendar td {
