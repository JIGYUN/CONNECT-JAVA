// filepath: src/app/issueComment/page.tsx
'use client';

import { useEffect, useMemo, useRef, useState, type ComponentType } from 'react';
import { useSearchParams } from 'next/navigation';
import { useissueCommentByDate, useUpsertissueComment } from '@/shared/issueComment';
import type { issueCommentEntry, issueCommentUpsertInput } from '@/shared/issueComment';
...
function pickHtmlSafe(v: issueCommentEntry | null | undefined): string {
    if (!v) return '';
    const bodyVal = (typeof v['content'] === 'string' && v['content'])
        || (typeof v['body'] === 'string' && v['body'])
        || '';
    return bodyVal;
}

export default function issueCommentPage() {
    ...
    const q: UseQueryResult<issueCommentEntry | null, Error> = useissueCommentByDate({
        diaryDt: selected,
        grpCd,
        ownerId,
    });

    const upsert: UseMutationResult<void, Error, issueCommentUpsertInput, unknown> = useUpsertissueComment({ grpCd, ownerId });
    ...
    useEffect(() => {
        ...
        if (cur !== content) {
            ...
            ed.setData(content || '');
            setTimeout(() => {
                settingByCode.current = false;
            }, 0);
        }
    }, [content]);

    const onSave = async () => {
        const body: issueCommentUpsertInput = {
            diaryDt: selected,
            content: content || '',
            grpCd,
            ownerId,
    ...
    return (
        <div className="issueComment-root" style={{ maxWidth: PAGE_MAX_WIDTH, margin: '0 auto', padding: 16 }}>
            {/* 헤더 */}
            ...
            <div className="calendar-wrap" style={{ border: '1px solid #e9edf3', borderRadius: 12, overflow: 'hidden', marginBottom: 12 }}>
                <table className="issueComment-calendar">
                ...
                {CKE ? (
                    <CKE.CKEditor
                        ...
                        onReady={(editor) => {
                            ...
                            view.change((writer) => {
                                writer.setStyle('min-height', `${MIN_EDITOR_HEIGHT}px`, root);
                            });
                        ...
                        config={{ placeholder: '내용을 작성하세요…', extraPlugins: [CustomUploadAdapterPlugin] }}
                    />
                ) : (
            ...
            <style jsx>{`
                ...
                .issueComment-calendar { width: 100%; table-layout: fixed; border-collapse: collapse; }
                .issueComment-calendar thead tr { background: #f8fafc; color: #0f172a; }
                .issueComment-calendar th { padding: 10px 0; font-weight: 700; }
                .issueComment-calendar td {
                ...
                .loadingPill{
                    ...
                    border:1px solid #e5e7eb; border-radius:999px; background:#f8fafc; color:#374151;
                    font-size:12px; margin-bottom:8px;
                }
                .loadingPill .dot{
                    width:8px; height:8px; border-radius:999px;
                    box-shadow:0 0 0 2px rgba(96,165,250,.25); background:#60a5fa;
                }
