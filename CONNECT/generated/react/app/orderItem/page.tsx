// filepath: src/app/orderItem/page.tsx
'use client';

import { useEffect, useMemo, useRef, useState, type ComponentType } from 'react';
import { useSearchParams } from 'next/navigation';
import { useOrderItemByDate, useUpsertOrderItem } from '@/shared/orderItem';
import type { OrderItemEntry, OrderItemUpsertInput } from '@/shared/orderItem';
...
function pickHtmlSafe(v: OrderItemEntry | null | undefined): string {
    if (!v) return '';
...
export default function OrderItemPage() {
    ...
    const q: UseQueryResult<OrderItemEntry | null, Error> = useOrderItemByDate({
        diaryDt: selected,
        grpCd,
        ownerId,
    });

    const upsert: UseMutationResult<void, Error, OrderItemUpsertInput, unknown> = useUpsertOrderItem({ grpCd, ownerId });
    ...
    const onSave = async () => {
        const body: OrderItemUpsertInput = {
            diaryDt: selected,
            content: content || '',
            grpCd,
            ownerId,
    ...
    return (
        <div className="orderItem-root" style={{ maxWidth: PAGE_MAX_WIDTH, margin: '0 auto', padding: 16 }}>
            {/* 헤더 */}
            ...
            <div className="calendar-wrap" style={{ border: '1px solid #e9edf3', borderRadius: 12, overflow: 'hidden', marginBottom: 12 }}>
                <table className="orderItem-calendar">
            ...
            <style jsx>{`
                ...
                .orderItem-calendar { width: 100%; table-layout: fixed; border-collapse: collapse; }
                .orderItem-calendar thead tr { background: #f8fafc; color: #0f172a; }
                .orderItem-calendar th { padding: 10px 0; font-weight: 700; }
                .orderItem-calendar td {
