// filepath: src/shared/payment/api/queries.ts
import { useQuery, useMutation, useQueryClient, type QueryKey } from '@tanstack/react-query';
import { postJson } from '@/shared/core/apiClient';
import type { PaymentEntry, PaymentUpsertInput } from '@/shared/payment/types';
import { adaptInPayment } from '@/shared/payment/adapters';
...
function keyPaymentByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): QueryKey {
    return ['payment/byDate', diaryDt, normGrp(grpCd), normOwner(ownerId)];
}

/** 서버 응답 → 첫 레코드 추출 */
function extractOne(v: unknown): PaymentEntry | null {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (Array.isArray(list)) {
            return list.length ? adaptInPayment(list[0]) : null;
        }
        if (
            isRec(cur) &&
    ...
    return isRec(cur) ? adaptInPayment(cur) : null;
}

const API = {
    selectByDate: '/api/pay/payment/selectPaymentByDate',
    upsert:       '/api/pay/payment/upsertPayment',
};

async function getByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): Promise<PaymentEntry | null> {
    const payload = {
        diaryDt,
        grpCd: normGrp(grpCd),
...
export function usePaymentByDate(p: { diaryDt: string; grpCd?: string | null; ownerId?: number | null }) {
    const diaryDt = p.diaryDt;
    const grpCd   = normGrp(p.grpCd ?? null);
    const ownerId = normOwner(p.ownerId ?? null);

    return useQuery<PaymentEntry | null, Error>({
        queryKey: keyPaymentByDate(diaryDt, grpCd, ownerId),
...
export function useUpsertPayment(ctx: { grpCd?: string | null; ownerId?: number | null }) {
    const qc = useQueryClient();
    const grpCd   = normGrp(ctx.grpCd ?? null);
    const ownerId = normOwner(ctx.ownerId ?? null);

    return useMutation<void, Error, PaymentUpsertInput>({
        ...
        onSuccess: (_d, v) => {
            ...
            if (keyStr) {
                qc.invalidateQueries({ queryKey: keyPaymentByDate(keyStr, grpCd, ownerId) });
            } else {
                qc.invalidateQueries({ queryKey: ['payment/byDate'] });
            }
        },
    });
}
