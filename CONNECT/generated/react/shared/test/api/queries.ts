// filepath: src/shared/test/api/queries.ts
import { useQuery, useMutation, useQueryClient, type QueryKey } from '@tanstack/react-query';
import { postJson } from '@/shared/core/apiClient';
import type { TestEntry, TestUpsertInput } from '@/shared/test/types';
import { adaptInTest } from '@/shared/test/adapters';
...
function keyTestByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): QueryKey {
    return ['test/byDate', diaryDt, normGrp(grpCd), normOwner(ownerId)];
}

/** 서버 응답 → 첫 레코드 추출 */
function extractOne(v: unknown): TestEntry | null {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (Array.isArray(list)) {
            return list.length ? adaptInTest(list[0]) : null;
        }
        if (
            isRec(cur) &&
    ...
    return isRec(cur) ? adaptInTest(cur) : null;
}

const API = {
    selectByDate: '/api/tst/test/selectTestByDate',
    upsert:       '/api/tst/test/upsertTest',
};

async function getByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): Promise<TestEntry | null> {
    const payload = {
        diaryDt,
        grpCd: normGrp(grpCd),
...
export function useTestByDate(p: { diaryDt: string; grpCd?: string | null; ownerId?: number | null }) {
    const diaryDt = p.diaryDt;
    const grpCd   = normGrp(p.grpCd ?? null);
    const ownerId = normOwner(p.ownerId ?? null);

    return useQuery<TestEntry | null, Error>({
        queryKey: keyTestByDate(diaryDt, grpCd, ownerId),
...
export function useUpsertTest(ctx: { grpCd?: string | null; ownerId?: number | null }) {
    const qc = useQueryClient();
    const grpCd   = normGrp(ctx.grpCd ?? null);
    const ownerId = normOwner(ctx.ownerId ?? null);

    return useMutation<void, Error, TestUpsertInput>({
        ...
        onSuccess: (_d, v) => {
            ...
            if (keyStr) {
                qc.invalidateQueries({ queryKey: keyTestByDate(keyStr, grpCd, ownerId) });
            } else {
                qc.invalidateQueries({ queryKey: ['test/byDate'] });
            }
        },
    });
}
