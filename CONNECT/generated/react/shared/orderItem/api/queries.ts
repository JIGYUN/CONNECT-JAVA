// filepath: src/shared/orderItem/api/queries.ts
import { useQuery, useMutation, useQueryClient, type QueryKey } from '@tanstack/react-query';
import { postJson } from '@/shared/core/apiClient';
import type { OrderItemEntry, OrderItemUpsertInput } from '@/shared/orderItem/types';
import { adaptInOrderItem } from '@/shared/orderItem/adapters';
...
function keyOrderItemByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): QueryKey {
    return ['orderItem/byDate', diaryDt, normGrp(grpCd), normOwner(ownerId)];
}

/** 서버 응답 → 첫 레코드 추출 */
function extractOne(v: unknown): OrderItemEntry | null {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (Array.isArray(list)) {
            return list.length ? adaptInOrderItem(list[0]) : null;
        }
        if (
            isRec(cur) &&
    ...
    return isRec(cur) ? adaptInOrderItem(cur) : null;
}

const API = {
    selectByDate: '/api/ord/orderItem/selectOrderItemByDate',
    upsert:       '/api/ord/orderItem/upsertOrderItem',
};

async function getByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): Promise<OrderItemEntry | null> {
    const payload = {
        diaryDt,
        grpCd: normGrp(grpCd),
...
export function useOrderItemByDate(p: { diaryDt: string; grpCd?: string | null; ownerId?: number | null }) {
    const diaryDt = p.diaryDt;
    const grpCd   = normGrp(p.grpCd ?? null);
    const ownerId = normOwner(p.ownerId ?? null);

    return useQuery<OrderItemEntry | null, Error>({
        queryKey: keyOrderItemByDate(diaryDt, grpCd, ownerId),
...
export function useUpsertOrderItem(ctx: { grpCd?: string | null; ownerId?: number | null }) {
    const qc = useQueryClient();
    const grpCd   = normGrp(ctx.grpCd ?? null);
    const ownerId = normOwner(ctx.ownerId ?? null);

    return useMutation<void, Error, OrderItemUpsertInput>({
        ...
        onSuccess: (_d, v) => {
            ...
            if (keyStr) {
                qc.invalidateQueries({ queryKey: keyOrderItemByDate(keyStr, grpCd, ownerId) });
            } else {
                qc.invalidateQueries({ queryKey: ['orderItem/byDate'] });
            }
        },
    });
}
