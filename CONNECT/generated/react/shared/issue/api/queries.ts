// filepath: src/shared/issue/api/queries.ts
import { useQuery, useMutation, useQueryClient, type QueryKey } from '@tanstack/react-query';
import { postJson } from '@/shared/core/apiClient';
import type { IssueEntry, IssueUpsertInput } from '@/shared/issue/types';
import { adaptInIssue } from '@/shared/issue/adapters';
...
function keyIssueByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): QueryKey {
    return ['issue/byDate', diaryDt, normGrp(grpCd), normOwner(ownerId)];
}

/** 서버 응답 → 첫 레코드 추출 */
function extractOne(v: unknown): IssueEntry | null {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (Array.isArray(list)) {
            return list.length ? adaptInIssue(list[0]) : null;
        }
        if (
            isRec(cur) &&
    ...
    return isRec(cur) ? adaptInIssue(cur) : null;
}

const API = {
    selectByDate: '/api/isu/issue/selectIssueByDate',
    upsert:       '/api/isu/issue/upsertIssue',
};

async function getByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): Promise<IssueEntry | null> {
    const payload = {
        diaryDt,
        grpCd: normGrp(grpCd),
...
export function useIssueByDate(p: { diaryDt: string; grpCd?: string | null; ownerId?: number | null }) {
    const diaryDt = p.diaryDt;
    const grpCd   = normGrp(p.grpCd ?? null);
    const ownerId = normOwner(p.ownerId ?? null);

    return useQuery<IssueEntry | null, Error>({
        queryKey: keyIssueByDate(diaryDt, grpCd, ownerId),
...
export function useUpsertIssue(ctx: { grpCd?: string | null; ownerId?: number | null }) {
    const qc = useQueryClient();
    const grpCd   = normGrp(ctx.grpCd ?? null);
    const ownerId = normOwner(ctx.ownerId ?? null);

    return useMutation<void, Error, IssueUpsertInput>({
        ...
        onSuccess: (_d, v) => {
            ...
            if (keyStr) {
                qc.invalidateQueries({ queryKey: keyIssueByDate(keyStr, grpCd, ownerId) });
            } else {
                qc.invalidateQueries({ queryKey: ['issue/byDate'] });
            }
        },
    });
}
