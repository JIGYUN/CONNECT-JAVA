...
/** 프런트 → 서버 (업서트/전송용) */
export function adaptOutChatRoom(input: ChatRoomEntry): Record<string, unknown> {
    return {
        ROOM_ID: input.id ?? null,
        GRP_CD: input.grpCd ?? null,
        OWNER_ID: input.ownerId ?? null,
        ROOM_NM: input.roomNm ?? null,
        ROOM_TYPE: input.roomType ?? null,
        ROOM_DESC: input.roomDesc ?? null,
        LAST_MSG_CONTENT: input.lastMsgContent ?? null,
        LAST_MSG_SENT_DT: input.lastMsgSentDt ?? null,
