...
function keyChatRoomByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): QueryKey {
...
const API = {
    selectByDate: '/api/cht/chat/selectChatRoomByDate',
    upsert:       '/api/cht/chat/upsertChatRoom',
};

async function getByDate(diaryDt: string, grpCd?: string | null, ownerId?: number | null): Promise<ChatRoomEntry | null> {
    const payload = {
        diaryDt,
        grpCd: normGrp(grpCd),
...
export function useChatRoomByDate(p: { diaryDt: string; grpCd?: string | null; ownerId?: number | null }) {
...
export function useUpsertChatRoom(ctx: { grpCd?: string | null; ownerId?: number | null }) {
    ...
    return useMutation<void, Error, ChatRoomUpsertInput>({
        ...
        onSuccess: (_d, v) => {
            ...
            if (keyStr) {
                qc.invalidateQueries({ queryKey: keyChatRoomByDate(keyStr, grpCd, ownerId) });
            } else {
